...
- (void)layoutSubviewsAnimated:(BOOL)animated
{
    ...
    tableFrame.size.height = CGRectGetHeight(self.superview.frame) - tableFrame.origin.y - [self _cellSize].height;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (count == 0) {
        ...
        if ([indexPath row] == 1) {
            [[cell textLabel] setText:NSLocalizedString(@"無工作時段", @"A label for a table with no events.")];
        }
        else
        {
